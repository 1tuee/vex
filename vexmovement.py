...
drivetrain = Drivetrain(lm, rm, 200, 320, 130, MM)
distance_sensor = Distance(Ports.PORT3)

def main():
    brain.screen.print("code init, working probably, leave the space ins")
    avoidstuff()

def avoidstuff():
    while True:
        diste = distance_sensor.distance(MM)
        if distance_sensor.is_object_detected() and diste < 200:
            # bac kup and turn
            drivetrain.stop()
